...
本项目采用纯`TypeScript V3.7.3`开发，自带声明文件，在基于`TypeScript`的项目中引用无需额外的声明！可以完全利用编辑器的智能提示！
...
文档和测试暂时没有足够时间完善！每次新功能完成都会推送到NPM仓库，`packages`目录下的每一个目录名都是一个包后缀，本仓库的所有包都是组织在`@clxx`的组织作用域中，如想使用`ImagePicker`组件，采用如下步骤：
